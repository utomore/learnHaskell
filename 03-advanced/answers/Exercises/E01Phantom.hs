-- | 第 1 章習題:Phantom types 與 DataKinds(參考解答)
...
-- | 產生下一個 ID。注意型別:Player 的下一個還是 Player 的,
-- 標籤在轉換過程中不會弄丟。
nextId :: Id e -> Id e
nextId (Id n) = Id (n + 1)
...
toFahrenheit (Temp c) = Temp (c * 9 / 5 + 32)
...
validateName (PlayerName raw)
  | T.null stripped = Left "名字不能為空"
  | T.length stripped > 12 = Left "名字太長"
  | otherwise = Right (PlayerName stripped)
  where
